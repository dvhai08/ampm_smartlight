...
const AMPM_CMD_PROCESS_TYPE smsSendCmdProcess_5	= {
		(void *)&smsSendCmdProcess_end,(void *)&smsSendMessage,Ampm_GsmSendCallback,Ampm_GsmRecvCallback,Ampm_GsmSmsSent,0,30000,100};

const AMPM_CMD_PROCESS_TYPE smsSendCmdProcess_4	= {
		(void *)&smsSendCmdProcess_5,(void *)&smsSendCmd_AT_CMGS,Ampm_GsmSendCallback,Ampm_GsmRecvCallback,NULL,0,10000,500};
...
uint8_t Ampm_GsmSmsRecvCallback_CCLK(uint8_t *buf)
{
  ...
  HAL_RTC_GetDate(&hrtc, &sdatestructureget, RTC_FORMAT_BIN);
	sysGsmTime.year = 2000 + sdatestructureget.Year;
	sysGsmTime.mday = sdatestructureget.Date;
	sysGsmTime.month = sdatestructureget.Month;
	sysGsmTime.hour = stimestructureget.Hours;
	sysGsmTime.min = stimestructureget.Minutes;
	sysGsmTime.sec = stimestructureget.Seconds;
	...
	if(osMutexWait(ampmGsmSmsMutexId, 1000) == osOK)
	{
		...
		while(msg != NULL)
		{
			...
			else
			{
				...
				if(msg->msg){ ampm_free(msg->msg);}
				if(msg) {ampm_free(msg);}
				if(msg)
				{
...
{
		struct SMS_LIST_TYPE *SMS = ampm_malloc(sizeof(struct SMS_LIST_TYPE));
		if(SMS == NULL)
			return SMS_ERROR;
		memset(SMS,0,sizeof(struct SMS_LIST_TYPE));
		if(mode != SMS_TEXT_MODE)
			SMS->msg = ampm_malloc(len);
		else
		{
			SMS->msg = ampm_malloc(len + 1);
			if(SMS->msg)
				SMS->msg[len] = '\0';
		}
		if(SMS->msg == NULL)
