...
osSemaphoreId sst25_sem_id;
osSemaphoreDef(SST25_SEM);

#define AMPM_FLAH_SPI_DBG(...)		//printf(__VA_ARGS__)

#define SST_CS_Assert()	HAL_GPIO_WritePin(SPI1_NSS_GPIO_Port, SPI1_NSS_Pin, GPIO_PIN_RESET)
#define SST_CS_DeAssert()	HAL_GPIO_WritePin(SPI1_NSS_GPIO_Port, SPI1_NSS_Pin, GPIO_PIN_SET)
...
uint8_t sst25buf[256] __attribute__ ((__section__("osRam")));

static uint8_t SST25__WriteEAR(uint32_t addr);

uint8_t OS_SST25_Erase(uint32_t addr, enum SST25_ERASE_MODE mode)
{
	uint8_t res = SST25_FAIL;
	if(osSemaphoreWait(sst25_sem_id, 3000) == osOK)
	{
		res = SST25_Erase(addr,mode);
		osSemaphoreRelease(sst25_sem_id);
	}
	return res;
}
...
uint8_t OS_SST25_Write(uint32_t addr, const uint8_t *buf, uint32_t count)
{
	...
	if(osSemaphoreWait(sst25_sem_id, 3000) == osOK)
	{
		res = SST25_Write(addr,buf,count);
		osSemaphoreRelease(sst25_sem_id);
	}
	return res;
}
...
uint8_t OS_SST25_WriteNoErase(uint32_t addr, const uint8_t *buf, uint32_t count)
{
	...
	if(osSemaphoreWait(sst25_sem_id, 3000) == osOK)
	{
		res = __SST25_Write(addr,buf,count);
		osSemaphoreRelease(sst25_sem_id);
	}
	return res;
}
...
uint8_t SST25_Init(void)
{
	...
	sst25_sem_id = osSemaphoreCreate(osSemaphore(SST25_SEM), 1);
