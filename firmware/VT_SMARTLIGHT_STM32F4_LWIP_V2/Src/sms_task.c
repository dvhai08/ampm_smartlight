...
void Ampm_MainSmsRecvCallback(uint8_t *buf)
{
	...
	if(FindAllPhone((char *)smsSender))
	{
			...
			if(Ampm_SendCommand("OK",modemError,1000,3,"AT\r")== AMPM_GSM_RES_OK)
			{
				if(Ampm_SendCommand("OK",modemError,1000,3,"AT+CSCS=\"UCS2\"\r")== AMPM_GSM_RES_OK)
				{
					...
					if(Ampm_SendCommand("+CUSD:",modemError,10000,3,"ATD*101#;\r") == AMPM_GSM_RES_OK)
					{
						...
						Ampm_RingingReset();
						sms_res_len += sprintf((char *)&sms_res[sms_res_len], "TKC=%s,", replySmsBuf);
					}
				}
			...
			if(Ampm_SendCommand("OK",modemError,1000,3,"AT\r")== AMPM_GSM_RES_OK)
			{
				if(Ampm_SendCommand("OK",modemError,1000,3,"AT+CSCS=\"UCS2\"\r")== AMPM_GSM_RES_OK)
				{
					...
					if(Ampm_SendCommand("+CUSD:",modemError,10000,3,"ATD*102#;\r") == AMPM_GSM_RES_OK)
					{
						...
						Ampm_RingingReset();
						sms_res_len += sprintf((char *)&sms_res[sms_res_len], "TKKM=%s,", replySmsBuf);
					}
				}
				Ampm_SendCommand("OK",modemError,1000,3,"AT+CSCS=\"GSM\"\r");
			}
		...
		pt = strstr(sms_in,(char *)"BATMOTNGORAO");
		if(pt != NULL)
		{
			deviceCfg.features |= DEFAULT_FEATURE_1_RELAY;
			DeviceCfg_Save();
			sms_res_len += sprintf(&sms_res[sms_res_len],"THANH CONG!");
			SMS_DBG("THANH CONG!");
		}
		
		pt = strstr(sms_in,(char *)"TATMOTNGORA");
		if(pt != NULL)
		{
			deviceCfg.features &= ~DEFAULT_FEATURE_1_RELAY;
			DeviceCfg_Save();
