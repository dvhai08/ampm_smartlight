...
/**************************************************************************************************/
/** the custom functions use to build json */
IoT_Error_t aws_iot_shahow_add_reported_header(char *pJsonDocument, size_t maxSizeOfJsonDocument)
{
	IoT_Error_t ret_val = AWS_SUCCESS;
	int32_t snPrintfReturn = 0;
	if(pJsonDocument == NULL)
	...
	size_t remSizeOfJsonBuffer = maxSizeOfJsonDocument - strlen(pJsonDocument);
...
IoT_Error_t aws_shadow_footer_reported(char *pJsonDocument, size_t maxSizeOfJsonDocument)
{
	IoT_Error_t ret_val = AWS_SUCCESS;
	int32_t snPrintfReturn = 0;
	if(pJsonDocument == NULL)
...
IoT_Error_t aws_iot_shadow_add_fields(char *pJsonDocument, size_t maxSizeOfJsonDocument,
	uint8_t count , ...)
{
	IoT_Error_t ret_val = AWS_SUCCESS;
	int32_t snPrintfReturn = 0;
	va_list pArgs;
	jsonStruct_t *pJson = NULL;
	va_start(pArgs, count);
	if(pJsonDocument == NULL)
		return NULL_VALUE_ERROR;

	size_t remSizeOfJsonBuffer = maxSizeOfJsonDocument;
	/* Is size of Json buffer legitimacy*/
	if(remSizeOfJsonBuffer <= 1)
		return SHADOW_JSON_ERROR;

	for(int i = 0; i < count; i++)
	{
		pJson = va_arg(pArgs, jsonStruct_t *);
		remSizeOfJsonBuffer = maxSizeOfJsonDocument - strlen(pJsonDocument);
		if(pJson != NULL)
		{
			snPrintfReturn = snprintf(pJsonDocument + strlen(pJsonDocument), remSizeOfJsonBuffer,
																			"\"%s\":", pJson->pKey);
			/* Is return value legitimacy*/
			ret_val = checkReturnValueOfSnPrintf(snPrintfReturn, remSizeOfJsonBuffer);
			if(ret_val != AWS_SUCCESS)
				return ret_val;
			if((pJson->pKey != NULL) && (pJson->pData != NULL))
			{
				remSizeOfJsonBuffer = maxSizeOfJsonDocument - strlen(pJsonDocument);
				ret_val = convertDataToString(pJsonDocument + strlen(pJsonDocument),
													remSizeOfJsonBuffer, pJson->type, pJson->pData);
			}
			else
				return NULL_VALUE_ERROR;
			if(ret_val != AWS_SUCCESS)
...
IoT_Error_t aws_iot_add_object(char *pJsonDocument, size_t maxSizeOfJsonDocument,
														jsonStruct_t *jsonObj, uint8_t count, ...)
{
	IoT_Error_t ret_val = AWS_SUCCESS;
	size_t remSizeOfJsonBuffer = maxSizeOfJsonDocument;
	int32_t snPrintfReturn = 0;
	...
	va_list pArgs;
	va_start(pArgs, count);
	if((pJsonDocument == NULL) && (jsonObj == NULL))
		return NULL_VALUE_ERROR;
	if(jsonObj->type != SHADOW_JSON_OBJECT)
		return FAILURE;
	/* Add json object*/
	remSizeOfJsonBuffer -= strlen(pJsonDocument);
	snPrintfReturn = snprintf(pJsonDocument + strlen(pJsonDocument), remSizeOfJsonBuffer,
																		"\"%s\":{", jsonObj->pKey);
	ret_val = checkReturnValueOfSnPrintf(snPrintfReturn, remSizeOfJsonBuffer);
	if(ret_val != AWS_SUCCESS)
...
IoT_Error_t aws_iot_add_object_from_buffer(char *pJsonDocument, size_t maxSizeOfJsonDocument,
														jsonStruct_t *jsonObj, char *buffer)
{
	IoT_Error_t ret_val = AWS_SUCCESS;
	size_t remSizeOfJsonBuffer = maxSizeOfJsonDocument;
	int32_t snPrintfReturn = 0;
...
int32_t FillWithClientTokenSize(char *pBufferToBeUpdatedWithClientToken, size_t maxSizeOfJsonDocument) {
	...
	snPrintfReturn = snprintf(pBufferToBeUpdatedWithClientToken, maxSizeOfJsonDocument, "%s-%d", mqttClientID,
							  clientTokenNum++);
	#else
	snPrintfReturn = snprintf(pBufferToBeUpdatedWithClientToken, maxSizeOfJsonDocument, "SML-%s",
...
static IoT_Error_t convertDataToString(char *pStringBuffer, size_t maxSizoStringBuffer, JsonPrimitiveType type,
	...
	}else if(type == SHADOW_JSON_ARRAY) {
		snPrintfReturn = snprintf(pStringBuffer, maxSizoStringBuffer, "[%s],", (char *) (pData));
	}
...
IoT_Error_t UpdateValueIfNoObject(const char *pJsonString, jsonStruct_t *pDataStruct, jsmntok_t token) {
	IoT_Error_t ret_val = AWS_SUCCESS;
	...
	}/*else if(pDataStruct->type == SHADOW_JSON_ARRAY) {
		ret_val = parseArrayValue((char *) pDataStruct->pData, pJsonString, &token);
	}*/
