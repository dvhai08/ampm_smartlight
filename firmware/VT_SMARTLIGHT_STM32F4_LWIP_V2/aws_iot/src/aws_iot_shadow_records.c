...
IoT_Error_t registerJsonTokenOnDelta(jsonStruct_t *pStruct) {
	...
	if(!deltaTopicSubscribedFlag) {
		...
		#else
		snprintf(shadowDeltaTopic, MAX_SHADOW_TOPIC_LENGTH_BYTES, "aws:%s/things/%s/shadow/update/delta", CLIENT_ORGRANIZATION_ID, myThingName);
		#endif
...
static void topicNameFromThingAndAction(char *pTopic, const char *pThingName, ShadowActions_t action,
	...
	#else
	if(SHADOW_ACTION == ackType) {
		snprintf(pTopic, MAX_SHADOW_TOPIC_LENGTH_BYTES, "aws:%s/things/%s/shadow/%s", CLIENT_ORGRANIZATION_ID, pThingName, actionBuf);
	} else {
		snprintf(pTopic, MAX_SHADOW_TOPIC_LENGTH_BYTES, "aws:%s/things/%s/shadow/%s/%s", CLIENT_ORGRANIZATION_ID, pThingName, actionBuf,
				 ackTypeBuf);
	}
	#endif
}
