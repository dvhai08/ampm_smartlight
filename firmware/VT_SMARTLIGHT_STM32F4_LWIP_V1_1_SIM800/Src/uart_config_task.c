...
uint32_t uart5_config_enable = 0;
uint32_t uart5_config_timeout = 0;
...
void UartConfigTask(void)
{
	...
	//if(CheckTimeout(&checkUart) == SYSTICK_TIMEOUT)
	{
		...
		//UART5 process
		if(uart5_config_timeout) uart5_config_timeout--;
		else uart5_config_enable = 0;
		...
		else
		{
			while(mUART5_GetChar(&c) == 0)
			{
					...
					{
						uart5_config_enable = 1;
						uart5_config_timeout = 30000; //30s
