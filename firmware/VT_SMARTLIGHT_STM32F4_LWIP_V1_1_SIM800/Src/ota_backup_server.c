...
void vOTA_Backup_Check(void)
{
	...
	{
		OTA_DBG("OTA: Firmware update timeout\r\n");
...
static void vOTA_Backup_Task(void const *param)
{
	...
	while(1)
	{
		...
		if((ret = lwip_net_connect(&ota_backup_socket,"fw.m2m.vn","8091",LWIP_NET_PROTO_TCP)) != 0)
		{
			OTA_DBG("OTA:lwip_net_connect fail\n");
			switch(ret)
			{
		...
		OTA_DBG("OTA:lwip_net_connect ok\n");
		OTA_DBG("OTA:send a login message\n");
		...
		while(1)
		{
			osDelay(10);
			vOTA_Backup_Check();
			OTA_DBG("OTA:lwip_net_recv_timeout\n");
			...
			{
				OTA_DBG("OTA:recv fail\n");
				osDelay(5000);
				break;
			}
			else if(recbytes > 0)
			{
				OTA_DBG("OTA:got %ld bytes\n",recbytes);
				...
				{
					OTA_DBG("OTA: Send %d bytes to server\r\n",GPRS_ProtoSend.length);
			...
			if(fSendBuff[0] == 0xCA)
			{
				...
				{
					OTA_DBG("OTA:send ota message interval\n");
					if (lwip_net_send(ota_backup_socket, (const unsigned char *)fSendBuff, fSendBuff[1] + 5) < 0) {
						lwip_net_free(&ota_backup_socket);
						OTA_DBG("OTA:send fail\n");
						osDelay(3000);
...
{
	ota_backup_stop();
	osThreadDef(ota_task, vOTA_Backup_Task, osPriorityNormal, 0, 512);
