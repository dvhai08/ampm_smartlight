...
osSemaphoreId app_config_sem_id;
osSemaphoreDef(APP_CONFIG_SEM);
...
void LoadFirmwareFile(void)
{
	...
	OS_SST25_Read(FIRMWARE_INFO_ADDR + FIRMWARE_STATUS_OFFSET*4,buff,4);
	firmwareStatus = *u32Pt;
	if(firmwareStatus == 0xA5A5A5A5)
	{
		OS_SST25_Read(FIRMWARE_INFO_ADDR + FIRMWARE_FILE_SIZE_OFFSET*4,buff, 4);
		firmwareFileSize = *u32Pt;
		if(firmwareFileSize >= FIRMWARE_SIZE_MAX)
		{
		...
		{
			OS_SST25_Read(FIRMWARE_INFO_ADDR+FIRMWARE_CRC_OFFSET*4,buff,4);
...
{
	app_config_sem_id = osSemaphoreCreate(osSemaphore(APP_CONFIG_SEM), 1);
	LoadFirmwareFile();
...
uint8_t CfgProcessData(CFG_PROTOCOL_TYPE *cfgProtoRecv,CFG_PROTOCOL_TYPE *cfgProtoSend,uint8_t *cfgSendDataBuff,uint32_t maxPacketSize,uint8_t logSendEnable)
{
	...
	CFG_DATA_TYPE	pt;
	if(osSemaphoreWait(app_config_sem_id, 3000) == osOK)
	{
	...
	// 					}
						if((maxPacketSize >= FLASH_PACKET_SIZE) && (u32Temp >= FLASH_PACKET_SIZE))
						{
								...
								{
									OS_SST25_Write(pt.packetNo + FIRMWARE_BASE_ADDR + i,pt.dataPt + i,FLASH_PACKET_SIZE);
									OS_SST25_Read(pt.packetNo + FIRMWARE_BASE_ADDR + i,buf, FLASH_PACKET_SIZE);
					...
					if(firmwareFileOffSet >= firmwareFileSize)
					{
						...
						{
							OS_SST25_Write(firmwareFileSize + FIRMWARE_BASE_ADDR + i,cfgSendDataBuff,FLASH_PACKET_SIZE);
						}
						for(i = 0; i < firmwareFileSize;i += FLASH_PACKET_SIZE)
						{
							OS_SST25_Read(i + FIRMWARE_BASE_ADDR,cfgSendDataBuff, FLASH_PACKET_SIZE);
							for(j = 0;j < FLASH_PACKET_SIZE;j++)
							{
	// 							u8pt = 0;
	...
	// 								DelayMs(1);
		...
		osSemaphoreRelease(app_config_sem_id);
	}
	return 0;
}
