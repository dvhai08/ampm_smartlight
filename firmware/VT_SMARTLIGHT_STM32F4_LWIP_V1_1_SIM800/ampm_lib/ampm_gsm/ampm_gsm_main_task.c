...
#ifdef CMSIS_OS_RTX 
#include "cmsis_os.h"
osThreadDef(AMPM_GSM_MainTask,AMPM_GSM_MainTask, osPriorityNormal,0, 1024);
#endif
/*
D =  AT CMD DEFAULT
...
const uint8_t ampm_AtGsmCmdStart[] = {
	...
	@@0$AT+CGSN\r$AT+CGSN\r\r\n$1$OK$ERROR$3$1000$100\r\n\
	@@0$AT+CIMI\r$AT+CIMI\r\r\n$6$OK$ERROR$3$3000$100\r\n\
	@@0$AT+CCID\r$AT+CCID\r\r\n$2$OK$ERROR$3$1000$100\r\n\
	@@0$AT+COPS=0,2\r$0$0$OK$NOT USE$20$3000$100\r\n\
	@@G$AT+CSQ\r$+CSQ:$4$OK$ERROR$1$2000$100\r\n\
	@@S"
};

const uint8_t ampm_AtGsmCmdStartSIM800[] = {
...
uint8_t ampmGsmPPPTryCnt = 0;
uint32_t smsCheckInterval = 60000;
enum PPPStatus
{
  PPP_CONNECTING,
  PPP_CONNECTED,
...
#ifdef PPP_LWIP
AMPM_FILEHANDLE pppStdin;
AMPM_FILEHANDLE pppStdout;

uint8_t pppBufIn[128];
uint8_t pppBufOut[128];
char *pppUser;
char *pppPass;
int thisPPP = -1;
#endif

#define AMPM_CHECK_SMS_INTERVAL 1800000 //30min
...
{
	ppp_status = PPP_DISCONNECTED;
	printf("Ampm_GsmInternetSetDisconnect\r\n");
	flagSystemStatus &= ~(SYS_SERVER_OK | SYS_GPRS_OK | SYS_SERVERS_MASH);
}

/* PPP status callback example */

static void status_cb(nsapi_error_t err_code) {
	ppp_status = PPP_DISCONNECTED;
}
...
static void ppp_output_cb(void)
{
	int iChar;
	while(1)
	{
		iChar = fgetc((FILE *)&pppStdout);
		if(iChar == EOF)
		{
			break;
		}
...
{
	if((ampmGSM_TaskPhase == AMPM_GSM_MAIN_DATACALL_PHASE) && (ppp_status == PPP_CONNECTED))
	{
		return 1;
	}
	return 0;
...
void AMPM_GSM_Init(char *apn, char *usr,char *pwr,uint8_t (*tcpIpCallback)(void),uint8_t (*tcpIpInit)(void))
{
	...
	AmpmFileHandle_Init((FILE *)&pppStdin,pppBufIn,sizeof(pppBufIn));
	AmpmFileHandle_Init((FILE *)&pppStdout,pppBufOut,sizeof(pppBufOut));
	pppStdout.callback = ppp_output_cb;
	mbed_lwip_core_init();
	#endif
	if(tcpIpCallback)
			...
			case AMPM_GSM_MAIN_STARTUP_PHASE:
				if(AMPM_GSM_Startup((const uint8_t *)ampm_AtGsmCmdStart)){
					...
					//if (flagGotSimCPOS && strstr((char *)gsmSimCPOSBuf, VIETTEL_PLMN)) {
						...
						//AmpmGsm_SendSmsText("0978779222","hello!");
//					}
//					else{
//						flagSystemStatus &= ~SYS_SIM_VIETTEL_OK;
//						ampmGSM_TaskPhase = AMPM_GSM_MAIN_INIT_PHASE;
...
//					}
			...
			case	AMPM_GSM_MAIN_DATACALL_PHASE:
				...
				#endif
				uint8_t c;
				fflush((FILE *)&pppStdin);
				while(COMM_Getc(&c) == 0)
				{
					if(fputc(c,(FILE *)&pppStdin) == EOF)
						break;
				}
				
				if(pppStdin.callback)
					pppStdin.callback();
				#ifdef PPP_UIP
				if(PPP_IsDead())
				#elif defined(PPP_LWIP)
				...
				if (!mbed_lwip_ppp_link_status())
				{
					if(ppp_status != PPP_CONNECTED)
					{
						mbed_lwip_ppp_link_set_connected();
						ppp_status = PPP_CONNECTED;
					}
				}
				else if((CheckTimeout(&tAmpmMainTimeoutTask) == SYSTICK_TIMEOUT))
				{
					ppp_status = PPP_DISCONNECTED;
				}
...
//					AMPM_GSM_LIB_DBG("AMPM_GSM_MAIN:AMPM_GSM_MAIN_IDLE_PHASE \r\n");
				...
				if(Ampm_GSM_DialUp_IsDone()){
					if(Ampm_GSM_DialUp_IsOk()){
						...
						*/
						AMPM_GSM_LIB_DBG("nsapi_ppp_connect\r\n");
						if(nsapi_ppp_connect((FILE *)&pppStdin,(FILE *)&pppStdout,status_cb,pppUser,pppPass,IPV4_STACK) != NSAPI_ERROR_OK)
						{
							AMPM_GSM_LIB_DBG("ppp_lwip_disconnect \r\n");
							ampmGSM_TaskPhase = AMPM_GSM_MAIN_IDLE_PHASE;
							AMPM_GSM_LIB_DBG("AMPM_GSM_MAIN:AMPM_GSM_MAIN_IDLE_PHASE \r\n");
							break;
						}
						AMPM_GSM_LIB_DBG("mbed_lwip_ppp_bringup \r\n");
						if(mbed_lwip_ppp_bringup() != NSAPI_ERROR_OK)
						{
							AMPM_GSM_LIB_DBG("ppp_lwip_disconnect \r\n");
							nsapi_ppp_disconnect();
							ampmGSM_TaskPhase = AMPM_GSM_MAIN_IDLE_PHASE;
						...
						InitTimeout(&tAmpmMainTimeoutTask,SYSTICK_TIME_SEC(15));
			...
			case	AMPM_GSM_MAIN_IDLE_PHASE:
				...
				else if(Ampm_SmsCheckMessage_IsEmpty() && ampm_NewSmsFlag)
				{
		...
		if(CheckTimeout(&tRingTask) == SYSTICK_TIMEOUT)
		{
			...
			)
			{
				...
				sysCfg.id[2] = '1';
				sysCfg.id[3] = '7';
				AMPM_GSM_LIB_DBG("AMPM_GSM_MAIN:**********NEW IMEI:%s***********",(char *)sysCfg.imei);
				strcpy((char *)&sysCfg.id[4],(char *)&sysCfg.imei[strlen((char *)sysCfg.imei) - 8]);
				AMPM_GSM_LIB_DBG("AMPM_GSM_MAIN:**********NEW DEVICE ID:%s******",(char *)sysCfg.id);
				CFG_Save();
			}
		//Get CCID
			if(flagGotSimCID)
...
uint32_t Ampm_GSM_GetATI(uint16_t cnt,uint8_t c)
{
	...
	if(FindData(&cmpSIM800,c) == 0)
	{
		#ifdef PPP_UIP
		PPP_SetAuthenticationLogin(1);
		#endif
		ampm_AtGsmCmdStart_pt = (uint8_t *)ampm_AtGsmCmdStartSIM800;
		AMPM_GSM_LIB_DBG("YOUR MODULE IS:Got ATI:%s \r\n",cmpSIM800.buff);
		ampmGsmModuleType = SIM800;
		return 0;
	}
