...
#define AMPM_CHECK_SMS_INTERVAL 600000 //2min
...
//					AMPM_GSM_LIB_DBG("AMPM_GSM_MAIN:AMPM_GSM_MAIN_IDLE_PHASE \r\n");
			...
			case	AMPM_GSM_MAIN_IDLE_PHASE:
				...
				else if(/*(smsNotBusyCnt >= 3) && Ampm_SmsCheckMessage_IsEmpty() && */
					(ampm_NewSmsFlag || SysTick_Get() - smsCheckTime > smsCheckInterval))
				{
		...
		if(
			(!Ampm_SmsCheckMessage_IsEmpty() 
				...
				|| SysTick_Get() - smsCheckTime > smsCheckInterval
			)
		)
		{
			Ampm_GsmGotoCmdMode();
		...
		if(CheckTimeout(&tRingTask) == SYSTICK_TIMEOUT)
		{
			...
			if(gsmSignal == 0)
			{
				smsCheckInterval = 60000;
			}
			else
			{
				smsCheckInterval = AMPM_CHECK_SMS_INTERVAL;
			}
