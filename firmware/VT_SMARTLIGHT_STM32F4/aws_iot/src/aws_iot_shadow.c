...
const ShadowInitParameters_t ShadowInitParametersDefault = {(char *) AWS_IOT_MQTT_HOST, 8883, NULL, NULL,
															NULL, false, NULL};
...
IoT_Error_t aws_iot_shadow_connect(AWS_IoT_Client *pClient, ShadowConnectParameters_t *pParams) {
	...
	ConnectParams.pPassword = pParams->pPassword;
	ConnectParams.passwordLen = pParams->passwordLen;
	ConnectParams.pUsername = pParams->pUsername;
	ConnectParams.usernameLen = pParams->usernameLen;
