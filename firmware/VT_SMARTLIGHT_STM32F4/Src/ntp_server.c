			...
			//	while(tryCnt < 3)
				{
					...
					{
						NTP_DBG("NTP_DBG:C > lwip_net_connect fail\n");
						switch(ret)
						{
					...
					NTP_DBG("NTP_DBG:C > lwip_net_connect ok\n");
...
void ntp_stop(void)
{
	NTP_DBG("NTP_DBG:C > ntp_stop\n");
	if(thread_id)
	{
		NTP_DBG("NTP_DBG:C > ntp kill task\n");
		osThreadTerminate(thread_id);
		if(ntp_socket > 0)
		{
			NTP_DBG("NTP_DBG:C > ntp kill socket %d\n",ntp_socket);
			lwip_net_free(&ntp_socket);
		}
	}
}
...
{
	ntp_stop();
	NTP_DBG("NTP_DBG:C > ntp_start\n");
	osThreadDef(ntp_task, vNTP_Task, osPriorityNormal, 0, 512);
