...
void CFG_Load(void)
{
	...
	//id
	if(sysCfg.id[0] == 0xff || strlen((char*)sysCfg.id) >= sizeof(sysCfg.id))
	{
		strcpy((char *)sysCfg.id,DEFAULT_ID);
		saveFlag = 1;
	}
	//firmwareVersion
	if(sysCfg.firmwareVersion[0] == 0xff || strlen((char*)sysCfg.firmwareVersion) >= sizeof(sysCfg.firmwareVersion))
	{
